...
def _silence_logs():
    ...
    for name in ("transformers", "urllib3", "datasets"):
...
def _unpack_batch_compat(trainer: Trainer, batch):
    """Tương thích Trainer._unpack_batch / _standardize_batch giữa các phiên bản."""
    if hasattr(trainer, "_unpack_batch"):
        return trainer._unpack_batch(batch)  # type: ignore
    return trainer._standardize_batch(batch)

# ========= Minimal per-epoch logger =========
class EpochSummaryCallback:
    """
    In tóm tắt mỗi epoch. Chỉ in khi isValPhase=True (kết thúc epoch) và có 'val_' trong logs.
    """
    def __init__(self):
        self.epoch_idx = 0

    def __call__(self, trainer: Trainer, logs: dict, isValPhase: bool):
        if not isValPhase or logs is None:
            return
        keys = [k for k in logs.keys() if k.startswith("val_")]
        if not keys:
            return
        self.epoch_idx += 1
        if QUIET:
            msg = f"[Epoch {self.epoch_idx}] " + " | ".join(
                [f"{k}={logs[k]:.4f}" for k in sorted(keys) if isinstance(logs[k], (int, float))]
            )
            print(msg)
        else:
            print(f"\n=== Epoch {self.epoch_idx} Summary ===")
...
def _build_cfg() -> Config:
    """
    ViDeBERTa + PhoWhisper + BiLSTM-Attn
    - checkpoint_dir: /root/MER/checkpoints/videberta_phowhisper_bilstm
    - DEFAULT CKPT paths (có thể override bằng env):
        VIDEBERTA_CKPT (mặc định '/root/MER/videberta-base'),
        PHOWHISPER_CKPT (mặc định '/root/MER/phowhisper-base')
    """
    ckpt_root = Path("/root/MER/checkpoints")
    _ensure_dir(ckpt_root)
    ckpt_dir = ckpt_root / "videberta_phowhisper_bilstm"

    # <== ĐÃ CẬP NHẬT MẶC ĐỊNH CHÍNH XÁC THEO YÊU CẦU
    text_ckpt = os.getenv("VIDEBERTA_CKPT", "/root/MER/videberta-base").strip()
    audio_ckpt = os.getenv("PHOWHISPER_CKPT", "/root/MER/phowhisper-base").strip()
    ...
    cfg = Config(
        ...
        audio_encoder_dim=768,  # sẽ overwrite bởi encoder.config.hidden_size
        audio_unfreeze=False,

        # (tuỳ chọn) override FE Whisper (nếu encoder hỗ trợ):
        whisper_n_fft=None,
        whisper_hop_length=None,
        whisper_win_length=None,
        whisper_nb_mels=None,

        # Fusion = BiLSTM Attention
...
def _new_trainer_and_optim(cfg: Config, num_training_steps: int) -> Tuple[Trainer, torch.optim.Optimizer, object]:
    ...
    # tạm thời đặt criterion, sẽ gán lại bằng class-weights ở dưới
...
def run_train_val(cfg: Config):
    ...
    collate = Collator(train_set.tokenizer, text_max_length=getattr(cfg, "text_max_length", 96))
    ...
    # Callbacks (best theo val_macro_f1) + epoch summaries
    ...
    epoch_log_cb = EpochSummaryCallback()
    callbacks = [ckpt_cb, epoch_log_cb]
...
def run_kfold(cfg: Config, K: int = 3, random_state: int = 42):
    ...
    Dùng StratifiedKFold để cân bằng lớp giữa các fold.
    """
    ...
    # Nhãn để stratify
    y = [label2id[it["emotion"]] for it in full.items]

    collate = Collator(full.tokenizer, text_max_length=getattr(cfg, "text_max_length", 96))
    ...
    for fold_id, (train_idx, val_idx) in enumerate(skf.split(indices, y), start=1):
        print(f"\n--- Fold {fold_id}/{K} ---")

        train_subset = Subset(full, train_idx.tolist())
        val_subset = Subset(full, val_idx.tolist())

        # Build train loader (bucket theo mix_lengths của subset)
        if bool(getattr(cfg, "use_length_bucket", True)):
        ...
        # class weights theo subset train của fold (bincount)
        y_train_fold = torch.tensor([y[i] for i in train_idx.tolist()], dtype=torch.long)
        binc = torch.bincount(y_train_fold, minlength=cfg.num_classes).float()
        binc[binc == 0] = 1.0
        inv = 1.0 / binc
        cw = inv * (cfg.num_classes / inv.sum())
        cw = cw.to(trainer.device)
        trainer.criterion = CrossEntropyLoss(cfg, weight=cw)

        # Callbacks cho fold: checkpoint + epoch summary
        ...
        trainer.fit(train_loader, epochs=cfg.num_epochs, eval_data=val_loader, callbacks=[ckpt_cb, epoch_log_cb])
...
def main():
    ...
    # 2) Chạy KFold evaluation (mặc định K=3 — có thể override bằng env KFOLD)
    k = int(os.getenv("KFOLD", "3"))
    if k > 0:
        run_kfold(cfg, K=k, random_state=42)

if __name__ == "__main__":
    # KFOLD=0 -> chỉ Train/Val; KFOLD>0 -> chạy KFold sau Train/Val
    main()
