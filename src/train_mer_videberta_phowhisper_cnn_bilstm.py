# src/train_mer_videberta_phowhisper_cnn_bilstm.py
import os
from pathlib import Path
import json
import torch

# Tái dùng Trainer + helpers từ src/train.py để nhất quán (AMP, clip grad, log…)
...
def main():
    # === Cấu hình chung ===
    ckpt_dir = Path("checkpoints/videberta_phowhisper_cnn_bilstm")
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    cfg = Config(
        # run metadata
        name="MER_ViDeBERTa_PhoWhisper_CNNBiLSTM",
        ...
        # optim & scheduler
        learning_rate=2e-4,           # LR cho head; encoder sẽ gán LR nhỏ hơn qua split_param_groups
        ...
        jsonl_dir="",
        sample_rate=16000,
        max_audio_sec=None,          # PhoWhisper sẽ tự truncate theo giới hạn 30s nếu cần
        text_max_length=96,
        ...
        # Text encoder → ViDeBERTa local mirror (đặt cạnh src/ như bạn đã tải)
        text_encoder_type="videberta",
        text_encoder_ckpt="videberta-base",  # thư mục local cạnh src/
        text_encoder_dim=768,
        text_unfreeze=False,                 # có thể dùng Gradual Unfreeze callback sau

        # Audio encoder → PhoWhisper
        audio_encoder_type="phowhisper",
        # Đặt về local mirror bạn đã tải, ví dụ "phowhisper-base" (đặt cạnh src/).
        # Nếu dùng model từ HF như "openai/whisper-small" thì đổi giá trị này.
        audio_encoder_ckpt="phowhisper-base",
        audio_encoder_dim=768,              # sẽ được ghi đè bởi encoder.config.hidden_size (d_model)
        audio_unfreeze=False,

        # (Tuỳ chọn) override tham số feature extractor Whisper (nếu encoder của bạn hỗ trợ):
        whisper_n_fft=None,
        ...
        # Lưu ý: cần có lớp FusionCnnBiLstmAttention và case "cnn_bilstm_attn" trong model/fusion/__init__.py & model/networks.py
        fusion_type="cnn_bilstm_attn",

        # BiLSTM hyper (dùng chung)
    ...
    # (Optional) mở dần encoder layers nếu muốn, mặc định để False để ổn định giai đoạn đầu
    ...
    # === Optimizer & Scheduler ===
    enc_lr, head_lr = 5e-6, float(cfg.learning_rate)  # LR nhỏ cho encoder, lớn hơn cho head
    ...
    # (Tuỳ chọn) nạp lại best weights nếu cần
    ...
    def collect_preds(_trainer, loader):
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                # chuyển text sang device (giống train_step)
    ...
    import h5py
    h5_name = "model_weights_videberta_phowhisper_cnn_bilstm.h5"
