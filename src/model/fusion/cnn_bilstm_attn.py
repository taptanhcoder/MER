...
class _TemporalCNN(nn.Module):
    ...
    def __init__(self, d_model: int, num_layers: int = 2, kernel_size: int = 3, dropout: float = 0.1, dilation_growth: int = 1):
        super().__init__()
        assert kernel_size % 2 == 1, "kernel_size nên là số lẻ để padding 'same' chuẩn"
        layers = []
        in_ch = d_model
        dilation = 1
        for _ in range(max(1, int(num_layers))):
            pad = (kernel_size // 2) * dilation
            conv = nn.Conv1d(in_ch, d_model, kernel_size=kernel_size, padding=pad, dilation=dilation, bias=True)
            layers += [conv, nn.GELU(), nn.Dropout(dropout)]
 
            in_ch = d_model
            dilation *= max(1, int(dilation_growth))
...
class FusionCnnBiLstmAttention(FusionStrategy):

    def __init__(self, cfg: Config):
        super().__init__(cfg)
        F = cfg.fusion_dim

        t_cnn_layers = int(getattr(cfg, "fusion_cnn_layers", 2))
        a_cnn_layers = int(getattr(cfg, "fusion_cnn_layers", 2))
        ksize = int(getattr(cfg, "fusion_cnn_kernel", 3))
        cnn_drop = float(getattr(cfg, "fusion_cnn_dropout", cfg.dropout))
        dil_growth = int(getattr(cfg, "fusion_cnn_dilation_growth", 1))

        self.text_cnn = _TemporalCNN(F, num_layers=t_cnn_layers, kernel_size=ksize, dropout=cnn_drop, dilation_growth=dil_growth)
        self.audio_cnn = _TemporalCNN(F, num_layers=a_cnn_layers, kernel_size=ksize, dropout=cnn_drop, dilation_growth=dil_growth)
    ...
    def forward(self, t, a, kpm_text=None, kpm_audio=None):
        ...
        t_c = self.text_cnn(t)   
        a_c = self.audio_cnn(a)  

        t_lstm = self._pack_lstm(t_c, kpm_text, self.text_lstm)
        a_lstm = self._pack_lstm(a_c, kpm_audio, self.audio_lstm)
